...
class DataWriter(enrichedList: List[Enriched]) {
  val outputPath = "/home/bd-user/IdeaProjects/DataEnrichment/output_filter.csv"
  var file: File = new File(outputPath)
  val output: FileWriter = new FileWriter(file)
  val writer: CSVWriter = new CSVWriter(output)
  val csvSchema: Array[String] = Array("Route_ID", "Service_ID", "Trip_ID", "Trip_Head_Sign", "Direction_ID",
    "Shape_ID", "Wheelchair_Accessible", "Note_FR", "Note_En", "Agency_ID", "Route_Short_Name", "Route_Long_Name",
    "Route_Type", "Route_URL", "Route_Colour", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday",
